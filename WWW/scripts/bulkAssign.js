...
// تابع برای باز کردن مودال تخصیص - نسخه اصلاح شده
export async function openAssignmentModal(assignmentId = null) {
    ...
    // منتظر بارگذاری شعبه‌ها بمانید
    try {
        await loadBranchesInLeads(); // حالا با await صبر می‌کنیم
    } catch (error) {
        console.error('خطا در بارگذاری شعبه‌ها:', error);
        showNotification('خطا در بارگذاری لیست شعبه‌ها', 'error');
        return;
    }

    // پر کردن dropdown شعبه‌ها
...
// تابع برای تنظیم event listenerهای بستن مودال
function setupModalCloseListeners() {
    ...
    // اضافه کردن event listener به همه دکمه‌های بستن
    ...
    // اگر دکمه انصراف جداگانه وجود دارد
    if (cancelButton) {
        cancelButton.addEventListener('click', closeModal);
    }
}

// تابع برای مدیریت فرم مودال
...
async function handleAssignmentFormSubmit() {
    try {
        ...
        // اعتبارسنجی
        if (!phone || !branchId) {
            showNotification('پر کردن فیلدهای شماره تماس و شعبه الزامی است', 'error');
            return;
        }
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta?.description || `خطا در ${assignmentId ? 'ویرایش' : 'افزودن'} تخصیص`);
        }

        const result = await response.json();
        
        if (result.meta && result.meta.is_success) {
            showNotification(result.meta.description || `تخصیص با موفقیت ${assignmentId ? 'ویرایش' : 'افزودن'} شد`, 'success');
            ...
            throw new Error(result.meta?.description || `خطا در ${assignmentId ? 'ویرایش' : 'افزودن'} تخصیص`);
        }
...
// تابع برای دریافت خلاصه تخصیص‌های شعب
...
export function initBulkAssignPage() {
    ...
    setupAssignmentFormListener(); // اضافه کردن این خط
