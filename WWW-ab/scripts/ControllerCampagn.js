...
export function initCampaignsTab() {
    ...
    // فتح النموذج لإضافة حملة
    document.getElementById('add-campaign-btn').addEventListener('click', function() {
        document.getElementById('campaign-modal-title').textContent = 'إضافة حملة جديدة';
...
export function renderCampaignsTable(campaigns) {
    ...
    campaigns.forEach(campaign => {
        ...
        const budget = campaign.budget ? new Intl.NumberFormat('fa-IR').format(campaign.budget) + ' تومان' : 'دون تغيير';
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 text-center whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${campaign.isactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    ${campaign.isactive ? 'نشيط' : 'عاجز'}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-campaign mr-3" data-id="${campaign.campaignid}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-campaign" data-id="${campaign.campaignid}">إزالة </button>
            </td>
        `;
...
export async function editCampaign(campaignId) {
    try {
        ...
        if (!response.ok) throw new Error('حدث خطأ أثناء تلقي معلومات الحملة');
        
        const data = await response.json();
        const campaign = data.data;
        
        // املأ النموذج بمعلومات الحملة
        document.getElementById('campaign-modal-title').textContent = 'تحرير الحملة';
        ...
        showNotification('حدث خطأ أثناء تلقي معلومات الحملة', 'error');
    }
}

export async function deleteCampaign(campaignId) {
    if (!confirm('هل أنت متأكد من إزالة هذه الحملة؟')) return;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta.description || 'حدث خطأ أثناء حذف الحملة');
        }
        
        showNotification('تم القضاء على الحملة بنجاح', 'success');
        loadCampaigns();
    } catch (error) {
