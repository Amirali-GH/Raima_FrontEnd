        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${branch.isactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    ${branch.isactive ? 'نشيط' : 'عاجز'}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-blue-600 hover:text-blue-900 view-branch mr-2" data-id="${branch.branchid}">التفاصيل </button>
                <button class="text-purple-600 hover:text-purple-900 edit-branch mr-2" data-id="${branch.branchid}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-branch" data-id="${branch.branchid}">إزالة </button>
            </td>
        `;
...
export function renderBranchesPagination(meta) {
    ...
    if (meta.page.page_num < meta.page.total_pages) {
        ...
        nextBtn.innerHTML = 'التالي &laquo;';
...
export async function editBranch(branchId) {
    try {
        ...
        document.getElementById('branch-modal-title').textContent = 'تحرير الفرع';
...
export async function deleteBranch(branchId) {
    if (!confirm('هل أنت متأكد من إزالة هذا الفرع؟')) return;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(responseData.meta?.description || 'خطأ في حذف الفرع');
        }
        
        showNotification(responseData.meta?.description || 'تم القضاء على الفرع بنجاح', 'success');
        loadBranches();
    } catch (error) {
        ...
        detailModal.innerHTML = `
            <div class="modal-content w-full max-w-2xl">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900">تفاصيل الفرع </h3>
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">رمز الفرع: </label>
                            <p class="text-sm text-gray-900">${data.code}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">الاسم الأصلي: </label>
                            <p class="text-sm text-gray-900">${data.mainname}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">الاسم الثانوي: </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">النظامية: </label>
                            <p class="text-sm text-gray-900">${data.issystemic ? 'نعم' : 'لا'}</p>
                        </div>
                        <div>
                ...
                <div class="flex justify-end mt-4">
                    <button type="button" class="btn-outline close-detail-modal">إغلاق </button>
                </div>
            </div>
        `;
