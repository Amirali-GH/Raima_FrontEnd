...
function validateImage(file) {
    if (!file.type.startsWith('image/')) {
        showNotification('يُسمح بملفات الفيديو فقط', 'error');
        return false;
    }
    if (file.size > 5 * 1024 * 1024) {
        showNotification(`ملف ${file.name} هو أكثر من 5MB`, 'error');
        return false;
    }
    return true;
}
...
export async function uploadImages() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('حدث خطأ أثناء تحميل الصور');
        }

        // محاكاة progress (خياري)
        let uploaded = 0;
...
export async function loadPastImageUploads() {
    ...
    try {
        ...
        if (!response.ok) throw new Error('حدث خطأ أثناء تحميل قائمة التحميل');
        ...
        if (!data.data || data.data.length === 0) {
            container.innerHTML = '<p class="text-center text-gray-500 py-4">لم يتم العثور على Aplood </p>';
            return;
        }
        ...
        data.data.forEach(upload => {
            ...
            card.innerHTML = `
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="font-medium text-gray-800">تحميل الصور </p>
                        <p class="text-sm text-gray-500">${formatDate(upload.uploadedAt)}</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="text-sm font-semibold text-purple-600">${upload.count} الصورة </span>
                </div>
            `;
        ...
        console.error(error);
        container.innerHTML = '<p class="text-center text-red-500 py-4">خطأ في التحميل </p>';
    }
}
