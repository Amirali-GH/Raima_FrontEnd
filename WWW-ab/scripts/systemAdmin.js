...
export function initSystemManagement() {
    ...
    document.querySelectorAll('.subtab-button').forEach(button => {
        button.addEventListener('click', function() {
            // تعطيل كافة المرؤوسين
            document.querySelectorAll('.subtab-button').forEach(btn => {
                btn.classList.remove('active', 'text-purple-600', 'border-purple-600');
                btn.classList.add('text-gray-500', 'border-transparent');
            });
            
            // تفعيل الترجمات الحالية
            this.classList.add('active', 'text-purple-600', 'border-purple-600');
            this.classList.remove('text-gray-500', 'border-transparent');
            
            // إخفاء كافة لوحات subtrass
            ...
            // تهيئة الوظائف المحددة لكل مرؤوس
            ...
            // الترجمة للعملاء من محددة سلفا initCustomersTab تمكنت
        });
    });    
...
// وظيفة مساعدة لخلق تأخير في البحث
