...
export function initSaleAgentsTab() {
    ...
    if (addButton) {
        addButton.addEventListener('click', function() {
            document.getElementById('saleagent-modal-title').textContent = 'إضافة خبير جديد';
...
export async function loadSaleAgents(page = 1, search = '', status = 'both') {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta?.description || 'خطأ في تلقي البيانات');
        }
    ...
    saleAgents.forEach(agent => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-blue-600 hover:text-blue-900 view-saleagent mr-2" data-id="${agent.employeeid}">التفاصيل </button>
                <button class="text-purple-600 hover:text-purple-900 edit-saleagent mr-2" data-id="${agent.employeeid}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-saleagent" data-id="${agent.employeeid}">إزالة </button>
            </td>
        `;
...
export function renderSaleAgentsPagination(meta) {
    ...
    const currentPage = parseInt(meta.page.page_num) || 1;
    const pageSize = parseInt(meta.page.page_size) || 20;
    const totalSize = parseInt(meta.page.total_size) || 0;
    const totalPages = Math.ceil(totalSize / pageSize);

    const maxVisiblePages = 5; // عدد أزرار الصفحة التي تظهر
    let startPage = Math.max(1, currentPage - Math.floor(maxVisiblePages / 2));
    let endPage = Math.min(totalPages, startPage + maxVisiblePages - 1);

    // تصحيح لعرض الصفحات الصحيحة
    ...
    if (currentPage > 1) {
        ...
        prevBtn.className = 'pagination-btn border border-gray-300 cursor-pointer';
        prevBtn.innerHTML = '&laquo; سابق';
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageBtn.className = `pagination-btn cursor-pointer ${i === currentPage ? 'bg-purple-500 text-white' : 'border border-gray-300'}`;
    ...
    if (currentPage < totalPages) {
        ...
        nextBtn.className = 'pagination-btn border border-gray-300 cursor-pointer';
        nextBtn.innerHTML = 'التالي &raquo;';
...
export async function saveSaleAgent() {
    try {
        ...
        const agentData = {
            employeeroleid: 1, // دور خبير المبيعات
...
export async function editSaleAgent(agentId) {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.meta?.description || 'خطأ في تلقي المعلومات');
        }
        
        const { data } = await response.json();
        
        document.getElementById('saleagent-modal-title').textContent = 'تحرير الخبراء';
...
export async function deleteSaleAgent(agentId) {
    if (!confirm('هل أنت متأكد من إزالة هذا الخبير؟')) return;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(responseData.meta?.description || 'خطأ في حذف الخبير');
        }
        
        showNotification(responseData.meta?.description || 'تم القضاء على الخبير بنجاح', 'success');
        loadSaleAgents();
    } catch (error) {
        ...
        detailModal.innerHTML = `
            <div class="modal-content w-full max-w-2xl">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900">تفاصيل الخبراء عن المبيعات </h3>
                    <button type="button" class="text-gray-400 hover:text-gray-500 close-detail-modal">
                        <i class="material-icons">close</i>
                    </button>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    <h4 class="text-md font-medium text-gray-700 mb-2">المعلومات الأصلية </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">رمز الخبير: </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">الرمز الوطني: </label>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-900">${data.isactive ? 'نشيط' : 'عاجز'}</p>
                        </div>
                ...
                <div class="flex justify-end mt-4">
                    <button type="button" class="px-4 py-2 border rounded-md close-detail-modal">إغلاق </button>
                </div>
            </div>
        `;
