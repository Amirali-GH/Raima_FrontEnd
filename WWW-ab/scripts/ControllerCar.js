...
export function initCarCompaniesSubTab() {
    ...
    document.getElementById('add-company-btn').addEventListener('click', () => {
        document.getElementById('company-modal-title').textContent = 'إضافة شركة جديدة';
...
export async function loadCarCompanies(page = 1, search = '', status = 'all') {
    try {
        ...
        showNotification('خطأ في استلام شركات السيارات', 'error');
    }
}
...
export async function loadCarCategories(page = 1, search = '', status = 'all') {
    try {
        ...
        if (!response.ok) throw new Error('خطأ في تلقي الفئات');
        ...
        showNotification('خطأ في تلقي فئات المركبات', 'error');
    }
}

export function populateCarCompanyDropdown() {
    const dropdown = document.getElementById('car-company');
    dropdown.innerHTML = '<option value="">اختيار الشركة </option>';
...
export function populateCarCategoryDropdown() {
    ...
    dropdown.innerHTML = '<option value="">اختيار التصنيف </option>';
...
export function renderCarsTable(cars) {
    ...
    if (!cars || cars.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" class="px-6 py-4 text-center text-gray-500">لم يتم العثور على العنصر </td></tr>`;
        return;
    }
    ...
    cars.forEach(car => {
        ...
        const price = car.baseprice ? new Intl.NumberFormat('fa-IR').format(car.baseprice) + ' تومان' : 'دون تغيير';
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${company.name || 'مجهول'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${category.name || 'مجهول'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${car.year || 'مجهول'}</td>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${car.isactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    ${car.isactive ? 'نشيط' : 'عاجز'}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-car mr-3" data-id="${car.carid}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-car" data-id="${car.carid}">إزالة </button>
            </td>
        `;
        tbody.appendChild(row);
    });
    
    // يضيف event listeners لتحرير وحذف الأزرار
...
export function renderCarCompaniesTable(companies) {
    ...
    if (companies.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">لم يتم العثور على العنصر </td></tr>`;
        return;
    }
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${company.isactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    ${company.isactive ? 'نشيط' : 'عاجز'}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-company mr-3" data-id="${company.id}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-company" data-id="${company.id}">إزالة </button>
            </td>
        `;
        tbody.appendChild(row);
    });
...
export function renderCarCategoriesTable(carCategories) {
    ...
    if (!carCategories || carCategories.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">لم يتم العثور على العنصر </td></tr>`;
        return;
    }
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${category.isactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    ${category.isactive ? 'نشيط' : 'عاجز'}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-category mr-3" data-id="${category.categoryid}">تحرير </button>
                <button class="text-red-600 hover:text-red-900 delete-category" data-id="${category.categoryid}">إزالة </button>
            </td>
        `;
        tbody.appendChild(row);
    });
...
export async function editCar(carId) {
    try {
        ...
        if (!response.ok) throw new Error('خطأ في تلقي معلومات السيارة');
        
        const data = await response.json();
        const car = data.data;
        
        // املأ النموذج بمعلومات السيارة
        document.getElementById('car-modal-title').textContent = 'تحرير السيارة';
        ...
        showNotification('خطأ في تلقي معلومات السيارة', 'error');
    }
}

export async function deleteCar(carId) {
    if (!confirm('هل أنت متأكد من إزالة هذه السيارة؟')) return;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta.description || 'خطأ في إزالة السيارة');
        }
        
        showNotification('تم القضاء على السيارة بنجاح', 'success');
        loadCars();
    } catch (error) {
...
export async function saveCar() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.meta?.description || 'خطأ في حفظ البيانات');
        }

        document.getElementById('car-modal').classList.add('hidden');
        showNotification('تم تخزين المعلومات بنجاح', 'success');
        loadCars();
    } catch (error) {
...
export async function editCarCategory(categoryId) {
    try {
        ...
        if (!response.ok) throw new Error('حدث خطأ أثناء تلقي معلومات التصنيف');
        
        const data = await response.json();
        const category = data.data;
        
        // Populate form with category data
        document.getElementById('category-modal-title').textContent = 'تحرير الفئة';
        ...
        showNotification('حدث خطأ أثناء تلقي معلومات التصنيف', 'error');
    }
}
...
export async function deleteCarCategory(categoryId) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.meta.description || 'خطأ في حذف التصنيف');
        }
        
        showNotification('تمت إزالة التصنيف بنجاح', 'success');
        loadCarCategories();
    } catch (error) {
