...
export function initCarsSubTab() {
    ...
    if (addCarBtn) {
        addCarBtn.addEventListener('click', function() {
            document.getElementById('car-modal-title').textContent = 'Add a new car';
...
export async function loadCarCompanies(page = 1, search = '', status = 'all') {
    try {
        ...
        showNotification('Error receiving car companies', 'error');
    }
}
...
export async function loadCarCategories(page = 1, search = '', status = 'all') {
    try {
        ...
        showNotification('Error in receiving car categories', 'error');
    }
}

export function populateCarCompanyDropdown() {
    const dropdown = document.getElementById('car-company');
    dropdown.innerHTML = '<option value="">Company selection</option>';
...
export function renderCarsTable(cars) {
    ...
    cars.forEach(car => {
        ...
        const price = car.baseprice ? new Intl.NumberFormat('en-US').format(car.baseprice) + ' Toman' : 'Not defined';
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${company.name || 'Uncertain'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${category.name || 'Uncertain'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${car.year || 'Uncertain'}</td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-car mr-3" data-id="${car.carid}">Edit</button>
                <button class="text-red-600 hover:text-red-900 delete-car" data-id="${car.carid}">Delete</button>
            </td>
        `;
...
export function renderCarCompaniesTable(companies) {
    ...
    if (companies.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No items found</td></tr>`;
        return;
    }
    ...
    companies.forEach(company => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-company mr-3" data-id="${company.id}">Edit</button>
                <button class="text-red-600 hover:text-red-900 delete-company" data-id="${company.id}">Delete</button>
            </td>
        `;
        tbody.appendChild(row);
    });
...
export function renderCarCategoriesTable(carCategories) {
    ...
    carCategories.forEach(category => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-category mr-3" data-id="${category.categoryid}">Edit</button>
                <button class="text-red-600 hover:text-red-900 delete-category" data-id="${category.categoryid}">Delete</button>
            </td>
        `;
        tbody.appendChild(row);
    });
...
export async function editCar(carId) {
    try {
        ...
        if (!response.ok) throw new Error('Error in receiving vehicle information');
        
        const data = await response.json();
        const car = data.data;
        
        // Fill the form with car information
        document.getElementById('car-modal-title').textContent = 'Car editing';
        ...
        showNotification('Error in receiving vehicle information', 'error');
    }
}
...
export async function deleteCar(carId) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta.description || 'Error deleting car');
        }
        
        showNotification('The vehicle was successfully removed', 'success');
        loadCars();
    } catch (error) {
...
export async function saveCar() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.meta?.description || 'Error saving data');
        }
...
export async function editCarCategory(categoryId) {
    try {
        ...
        if (!response.ok) throw new Error('Error in receiving category information');
        ...
        showNotification('Error in receiving category information', 'error');
    }
}
...
export async function deleteCarCategory(categoryId) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.meta.description || 'Error deleting category');
        }
