...
export function initCampaignsTab() {
    ...
    // Open the modal to add a campaign
    document.getElementById('add-campaign-btn').addEventListener('click', function() {
        document.getElementById('campaign-modal-title').textContent = 'Add a new campaign';
...
export function renderCampaignsTable(campaigns) {
    ...
    campaigns.forEach(campaign => {
        ...
        const budget = campaign.budget ? new Intl.NumberFormat('en-US').format(campaign.budget) + ' Toman' : 'Not defined';
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-gray-500">${new Date(campaign.startdate).toLocaleDateString('en-US')}</td>
            <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-gray-500">${new Date(campaign.enddate).toLocaleDateString('en-US')}</td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-purple-600 hover:text-purple-900 edit-campaign mr-3" data-id="${campaign.campaignid}">Edit</button>
                <button class="text-red-600 hover:text-red-900 delete-campaign" data-id="${campaign.campaignid}">Delete</button>
            </td>
        `;
...
export async function editCampaign(campaignId) {
    try {
        ...
        if (!response.ok) throw new Error('Error in receiving campaign information');
        
        const data = await response.json();
        const campaign = data.data;
        
        // Fill out the form with campaign information
        document.getElementById('campaign-modal-title').textContent = 'Campaign editing';
        ...
        showNotification('Error in receiving campaign information', 'error');
    }
}
...
export async function deleteCampaign(campaignId) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.meta.description || 'Error deleting campaign');
        }
        
        showNotification('The campaign was successfully deleted', 'success');
        loadCampaigns();
    } catch (error) {
